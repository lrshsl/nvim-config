...
function ToggleTerminal()
   ...
   if TermWin == nil or not vim.api.nvim_win_is_valid(TermWin) then
      ...
      if not is_initialized then
         is_initialized = true
         vim.api.nvim_buf_set_option(TermBuf, 'modified', false)

         -- Store the channel ID globally here
         TermChan = vim.fn.termopen("fish --init-command=\"builtin cd " .. vim.fn.getcwd() .. "\"")
      end
...
--- @param execute_directly boolean: If true, appends \n to run the command immediately
function Terminal_SendCommandAndFocus(cmd, execute_directly)
   -- Default to true if not provided
   if execute_directly == nil then execute_directly = true end

   -- 1. Ensure the terminal window is open and focused
   if TermWin == nil or not vim.api.nvim_win_is_valid(TermWin) then
      ToggleTerminal()
   else
   ...
   if TermChan then
      vim.api.nvim_chan_send(TermChan, keys)
   else
      print("Terminal channel not found. Is the terminal initialized?")
   end

   -- 4. Move to insert mode so you're ready to type/edit
   vim.api.nvim_command('startinsert')
end
